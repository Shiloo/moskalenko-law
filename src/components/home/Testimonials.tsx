...
export default function Testimonials() {
  ...
  return (
    <section className="py-20 lg:py-28 relative" style={{ background: "var(--bg-section-alt)" }}>
        ...
        <div className="hidden lg:grid grid-cols-2 gap-5">
          {items.map((item, i) => (
            <AnimatedSection key={i} delay={i * 0.1}>
              <div
                className="card p-7 relative group hover:shadow-lg transition-all duration-300"
                style={{ borderColor: i === 0 ? "rgba(196, 146, 42, 0.25)" : undefined }}
              >
                <div className="absolute top-5 right-6 opacity-[0.07]" style={{ color: "#C4922A" }}>
                  <Quote size={44} />
                </div>
                <div className="flex gap-0.5 mb-4">
                  {Array.from({ length: item.rating }).map((_, j) => (
                    <span key={j} style={{ color: "#D4A84B" }}>★</span>
                  ))}
                </div>
                <p className="text-sm leading-relaxed mb-5" style={{ color: "#445066" }}>
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <div className="text-sm font-bold" style={{ color: "#1A2744" }}>{item.name}</div>
                    <div className="text-xs" style={{ color: "#C4922A" }}>{item.case}</div>
                  </div>
                </div>
              </div>
            </AnimatedSection>
          ...
          <AnimatePresence mode="wait">
            ...
            >
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  <div className="text-sm font-bold" style={{ color: "#1A2744" }}>{items[current].name}</div>
                  <div className="text-xs" style={{ color: "#C4922A" }}>{items[current].case}</div>
                </div>
              </div>
            </motion.div>
          </AnimatePresence>
              ...
              {items.map((_, i) => (
                <button
                  ...
                  style={{
                    ...
                    height: "8px",
                    background: i === current ? "#C4922A" : "rgba(196, 146, 42, 0.2)",
                  }}
                />
              ))}
            </div>
