...
import GavelIllustration from "@/components/ui/GavelIllustration";
      ...
      <section className="py-16 lg:py-20" style={{background:"#FFFFFF"}}>
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="flex justify-center mb-12">
            <AnimatedSection>
              <GavelIllustration className="w-48 h-auto opacity-80" />
            </AnimatedSection>
          </div>
          ...
          <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-5">
            {items.map((item,i)=>(
              <AnimatedSection key={i} delay={i*0.1}>
                <div className="relative rounded-2xl p-6 h-full flex flex-col transition-all duration-300 card-hover"
                  style={item.highlight?{
                    background:"linear-gradient(135deg,#0F1B4D 0%,#1E3A8A 100%)",
                    border:"none",boxShadow:"0 16px 40px rgba(15,27,77,0.25)",
                  }:{background:"white",border:"1px solid rgba(37,99,235,0.1)",boxShadow:"var(--shadow-card)"}}>
                  {item.highlight&&(
                    <div className="absolute -top-3 left-1/2 -translate-x-1/2 px-4 py-1 rounded-full text-xs font-bold text-white"
                      style={{background:"linear-gradient(135deg,#1D4ED8,#3B82F6)"}}>★ Popular</div>
                  )}
                  <div className="mb-5">
